...
define('MAX_PHOTO_SIZE', 5 * 1024 * 1024); // 5MB instead of 200KB
...
class ProjectManagementSystem {
    ...
    -- ✅ New Fields (added here, BEFORE constraints)
    criticality VARCHAR(10), -- PG0, PG1, PG2
    address TEXT,
    actual_carpet_area_sqft DECIMAL(10,2),
    token_released VARCHAR(10), -- 'Yes' or 'No'
    power_availability_kva VARCHAR(50),

    -- ✅ Foreign Key Constraint (at the end)
    FOREIGN KEY (created_by) REFERENCES users(id)
            ...
            // Tasks table - Updated with extensive new fields
    ...
    private function createDefaultAdmin() {
        ...
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM users WHERE role = 'Super Admin'");
        $stmt->execute();
        $superAdminCount = $stmt->fetchColumn();

        if ($superAdminCount == 0) {
            $stmt = $this->db->prepare("INSERT INTO users (email, password, role) VALUES (?, ?, ?)");
            $stmt->execute(['superadmin@example.com', password_hash('superpass', PASSWORD_DEFAULT), 'Super Admin']);
        }
    }
    ...
    try {
        switch (true) {
            ...
            case $path === '/state-city-data':
                ...
                if ($method === 'POST') $this->updateStateCityData();
                break;
            case $path === '/state-city-data/export':
                if ($method === 'GET') $this->exportStateCityCSV();
                break;
            case $path === '/state-city-data/add':
                if ($method === 'POST') $this->addStateCityRow();
                break;
            case $path === '/state-city-data/raw':
                if ($method === 'GET') $this->getStateCityDataForEditing();
                break;
            // Handle dynamic delete route
            case preg_match('/^\/state-city-data\/(\d+)$/', $path, $matches):
                if ($method === 'DELETE') $this->deleteStateCityRow($matches[1]);
                break;
            ...
            case $path === '/route/state-city-data':
                ...
                if ($method === 'POST') $this->updateStateCityData();
                break;
            case $path === '/route/state-city-data/update':
                if ($method === 'POST') $this->updateStateCityData();
                break;
            case $path === '/route/state-city-data/export':
                if ($method === 'GET') $this->exportStateCityCSV();
                break;
            case $path === '/route/state-city-data/add':
                if ($method === 'POST') $this->addStateCityRow();
                break;
            case $path === '/route/state-city-data/raw':
                if ($method === 'GET') $this->getStateCityDataForEditing();
                break;
            // Handle dynamic delete route with /route/ prefix
            case preg_match('/^\/route\/state-city-data\/(\d+)$/', $path, $matches):
                if ($method === 'DELETE') $this->deleteStateCityRow($matches[1]);
                break;
...
private function getBearerToken() {
    ...
    private function login() {
        ...
        // Handle both email/phone identifier and direct email/phone fields
        $identifier = isset($input['identifier']) ? $input['identifier'] : null;
        $email = isset($input['email']) ? $input['email'] : $identifier;
        $phone = isset($input['phone']) ? $input['phone'] : $identifier;
        
        if ((!$email && !$phone) || !isset($input['password'])) {
            $this->sendError('Email/phone and password required');
        }
        ...
        // If not found by email, try phone (if phone field exists in users table)
        if (!$user && $phone) {
...
foreach ($projects as $project) {
   ...
   private function createTask() {
    ...
    private function updateTask($taskId) {
        ...
        // Check permissions - allow users to update tasks they're assigned to
        if ($user['role'] !== 'Admin' && $user['role'] !== 'Super Admin' && $user['role'] !== 'Editor') {
            if ($task['assigned_to'] != $user['id'] && $task['created_by'] != $user['id']) {
                $this->sendError('Access denied to update this task', 403);
            }
            // Normal users can only update status and progress of tasks they're assigned to or created
            $allowedFields = [
    ...
    private function uploadMedia() {
        ...
        for ($i = 0; $i < count($_FILES['media']['name']); $i++) {
            ...
            if (in_array($fileType, ['jpg', 'jpeg', 'png']) && $fileSize > MAX_PHOTO_SIZE) {
                $errors[] = "Photo file $fileName exceeds maximum size of 5MB";
                continue;
            }
...
private function getStateCityData() {
    ...
    try {
        ...
        foreach ($lines as $lineNumber => $line) {
            ...
            // Skip if essential data is missing (only need state, town name is optional)
            if (empty($state)) {
                error_log("Skipping line " . ($lineNumber + 2) . ": empty state - State: '$state'");
                $errorCount++;
                continue;
            }
            ...
            // Only add city/town if town name exists
            if (!empty($townName)) {
    ...
    private function updateStateCityData() {
        $input = json_decode(file_get_contents('php://input'), true);
        
        if (!isset($input['data']) || !is_array($input['data'])) {
            $this->sendError('Data array is required');
        }
        ...
        try {
            // Prepare CSV content
            $csvContent = "State,State_Code,District_Code,District_Name,Town_Code,Town_Name\n";
            
            foreach ($input['data'] as $row) {
                // Ensure all required fields exist
                $state = isset($row['State']) ? $row['State'] : '';
                $stateCode = isset($row['State_Code']) ? $row['State_Code'] : '';
                $districtCode = isset($row['District_Code']) ? $row['District_Code'] : '';
                $districtName = isset($row['District_Name']) ? $row['District_Name'] : '';
                $townCode = isset($row['Town_Code']) ? $row['Town_Code'] : '';
                $townName = isset($row['Town_Name']) ? $row['Town_Name'] : '';
                
                // Escape CSV fields
                $csvRow = [
                    $this->escapeCsvField($state),
                    $this->escapeCsvField($stateCode),
                    $this->escapeCsvField($districtCode),
                    $this->escapeCsvField($districtName),
                    $this->escapeCsvField($townCode),
                    $this->escapeCsvField($townName)
                ];
            ...
            if (file_put_contents($csvFilePath, $csvContent) === false) {
                $this->sendError('Failed to write to CSV file', 500);
            }
            
            $this->sendResponse([
                'message' => 'State-City data updated successfully',
                'records_count' => count($input['data'])
            ]);
            
        } catch (Exception $e) {
            $this->sendError('Failed to update State-City data: ' . $e->getMessage(), 500);
        }
    }
    ...
    private function exportStateCityCSV() {
        $csvFilePath = __DIR__ . '/data/StateCity.csv';
        ...
        try {
            ...
            header('Content-Disposition: attachment; filename="StateCity.csv"');
            ...
            $this->sendError('Failed to export CSV: ' . $e->getMessage(), 500);
        }
    }

    private function addStateCityRow() {
        $input = json_decode(file_get_contents('php://input'), true);
        
        if (!isset($input['State']) || !isset($input['State_Code']) || !isset($input['District_Code']) || 
            !isset($input['District_Name']) || !isset($input['Town_Code']) || !isset($input['Town_Name'])) {
            $this->sendError('All required fields must be provided: State, State_Code, District_Code, District_Name, Town_Code, Town_Name');
        }
        ...
        try {
            ...
            $isNewFile = !file_exists($csvFilePath);
            if ($isNewFile) {
                $header = "State,State_Code,District_Code,District_Name,Town_Code,Town_Name\n";
                file_put_contents($csvFilePath, $header);
            }

            // Prepare the new row
            $newRow = [
                $this->escapeCsvField($input['State']),
                $this->escapeCsvField($input['State_Code']),
                $this->escapeCsvField($input['District_Code']),
                $this->escapeCsvField($input['District_Name']),
                $this->escapeCsvField($input['Town_Code']),
                $this->escapeCsvField($input['Town_Name'])
            ];

            $csvLine = implode(',', $newRow) . "\n";

            // Append to file
            if (file_put_contents($csvFilePath, $csvLine, FILE_APPEND) === false) {
                $this->sendError('Failed to write to CSV file', 500);
            }

            $this->sendResponse([
                'message' => 'State-City row added successfully',
                'data' => $input
            ]);

        } catch (Exception $e) {
            $this->sendError('Failed to add State-City row: ' . $e->getMessage(), 500);
        }
    }

    private function getStateCityDataForEditing() {
        $csvFilePath = __DIR__ . '/data/StateCity.csv';
        ...
        try {
            ...
            $data = [];
            $headers = ['State', 'State_Code', 'District_Code', 'District_Name', 'Town_Code', 'Town_Name'];

            // Skip header line if it exists
            $dataLines = $lines;
            if (!empty($dataLines) && strpos($dataLines[0], 'State') === 0) {
            ...
            foreach ($dataLines as $line) {
                ...
                $data[] = [
                    'State' => trim($row[0] ?? ''),
                    'State_Code' => trim($row[1] ?? ''),
                    'District_Code' => trim($row[2] ?? ''),
                    'District_Name' => trim($row[3] ?? ''),
                    'Town_Code' => trim($row[4] ?? ''),
                    'Town_Name' => trim($row[5] ?? '')
                ];
            }
            ...
            $this->sendError('Failed to get State-City data for editing: ' . $e->getMessage(), 500);
        }
    }

    private function deleteStateCityRow($index) {
        $csvFilePath = __DIR__ . '/data/StateCity.csv';
        ...
        try {
            ...
            if ($index < 0 || $index >= count($lines)) {
                $this->sendError('Invalid row index', 400);
            }

            // Remove the specified row (considering header)
            $adjustedIndex = $index;
            if (strpos($lines[0], 'State') === 0) {
                $adjustedIndex++; // Skip header
            }

            if ($adjustedIndex >= count($lines)) {
                $this->sendError('Invalid row index', 400);
            }

            // Remove the row
            array_splice($lines, $adjustedIndex, 1);

            // Write back to file
            $newContent = implode("\n", $lines);
            if (file_put_contents($csvFilePath, $newContent) === false) {
                $this->sendError('Failed to update CSV file', 500);
            }

            $this->sendResponse([
                'message' => 'State-City row deleted successfully',
                'deleted_index' => $index
            ]);
